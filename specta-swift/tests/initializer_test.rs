...
fn test_initializer_generation() {
    let mut types = TypeCollection::default().register::<TestStruct>();
    ...
    // Verify that the memberwise initializer is NOT present
    // (Codable initializers like `init(from decoder:)` are still allowed)
    assert!(
        !result_without_init.contains("public init(jobId:")
            && !result_without_init.contains("public init(includeStats:")
    );
}
